...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const PROJECTS = [
    {
      ...
      overview: 'Segmented 5,360 customers from a 1M+ row UK e-commerce dataset using RFM (Recency, Frequency, Monetary) methodology in PostgreSQL — turning raw transactional data into a clear, actionable customer tier map.',
      stats: [
      ...
      tools: ['PostgreSQL', 'CTEs', 'Window Functions', 'NTILE', 'Excel', 'Donut Charts', 'Bar Charts', 'RFM Methodology'],
      outcome: '<strong>Outcome:</strong> Delivered a segmentation framework that pinpointed 461 At Risk and 1,035 Lost customers — enabling targeted retention and re-engagement campaigns and giving the marketing team a reusable scoring pipeline.',
      // Replace image src values with your actual screenshot files in the ./images/ folder
      images: [
        { src: './images/rfm-dashboard.JPG',  alt: 'RFM Segmentation Dashboard' },
    ...
    },
    {
      ...
      overview: 'A statistical deep-dive into sales data combining Python for exploratory analysis and Power BI for executive dashboards — surfacing the revenue distribution pattern and quantifying the relationship between units sold and sales amount.',
      stats: [
      ...
      tools: ['Power BI', 'Python', 'Pandas', 'NumPy', 'Statistics', 'Confidence Intervals', 'KPI Cards', 'DAX'],
      outcome: '<strong>Outcome:</strong> Provided management with a statistically-backed dashboard (avg sales ≈ 4,136; 95% CI: 3,781–4,491) that replaced gut-feel reporting with data-driven business decisions.',
      images: [
        { src: './images/sales-dashboard.png',  alt: 'Power BI KPI Dashboard' },
        { src: './images/revenue-chart.png',     alt: 'Revenue Distribution Chart' },
    ...
    },
    {
      ...
      overview: 'An end-to-end analytics pipeline processing 10+ years of India trade data across 50+ commodity categories — from raw data ingestion in Python through optimised PostgreSQL storage to an interactive Power BI dashboard uncovering key trade partners and commodity trends.',
      stats: [
      ...
      tools: ['Python', 'Pandas', 'PostgreSQL', 'Query Optimisation', 'Indexing', 'CTEs', 'Power BI', 'DAX', 'Data Cleaning'],
      outcome: '<strong>Outcome:</strong> Delivered a reusable pipeline and interactive dashboard revealing decade-long trade patterns — showing China, USA, and UAE as dominant partners — with a 25% boost in query performance through schema optimisation.',
      images: [
        { src: './images/trade-dashboard.png',   alt: 'Trade Dashboard Overview' },
        { src: './images/partner-analysis.png',  alt: 'Partner Country Analysis' },
  ...
  function openModal(projectId) {
    ...
    // stats
    modalStats.innerHTML = p.stats.map(s => `
    ...
    // gallery
    modalGallery.innerHTML = p.images.map(img => `
    ...
    // tools
    modalTools.innerHTML = p.tools.map(t => `<span class="modal-tool-chip">${t}</span>`).join('');
